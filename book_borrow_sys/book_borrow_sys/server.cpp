...
bool add_books_info(TCPServer &server,int conn_fd,char *recv_data)
{
    ...
    if(db.result && mysql_num_rows(db.result) == 0){
        ...
        */
        sentence = "insert book_infor(ISBN,book_name,publish_house,author,count,stat,introduce,taga,tagb) value(\"" + book["ISBN"].asString()+ "\","+"\"" + book["book_name"].asString()+"\","+"\"" + book["publish_house"].asString()+"\"," + "\""+book["author"].asString()+"\"," + book["count"].asString() + ",\"" +book["stat"].asString()+"\",\""+book["introduce"].asString()+"\",\"" + book["taga"].asString()+"\",\""+ book["tagb"].asString() + "\"" + ");";
    }else{
        //这本书本来就存在
